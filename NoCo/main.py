...
class Runner:
    ...
    def model_choise(self):
        assert self.args.model_name in ["EVA", "MCLEA", "MSNEA", "NoCo"]
        if self.args.model_name == "NoCo":
            self.model = NoCo(self.KGs, self.args)
    ...
    def run(self):
        ...
        self.curr_loss = 0.  # 当前损失
        self.lr = self.args.lr  # 学习率
        self.curr_loss_dic = defaultdict(float)  # 损失字典
        ...
        with tqdm(total=self.args.epoch) as _tqdm:
            for i in range(self.args.epoch):  # 训练循环
    ...
    def il_for_ea(self):
        with torch.no_grad():
            if self.args.model_name in ["NoCo"]:
